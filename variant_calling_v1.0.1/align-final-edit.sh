...
usage() {
    echo "Usage: $0 --genome-fasta <genome_fasta> --chromosome <chromosome> --output-dir <output_dir> --fastq1 <fastq1> [--fastq2 <fastq2>] [--aligner <bowtie2|bwa|STAR>] [--assaytype <RNA-seq|ATAC-seq|Other-seq>]"
    echo ""
    echo "Arguments:"
    ...
    echo " --aligner <bowtie2|bwa|STAR> Aligner to use. Options: 'bowtie2' or 'bwa'. Defaults to 'STAR'."
    echo " --assaytype <RNA-seq|ATAC-seq|Other-seq> Type of assay. Options: 'RNA-seq', 'ATAC-seq' or 'Other-seq'. Defaults to 'RNA-seq'."
...
GTF_ANNOTFILE="/data/abattle4/surya/datasets/for_lakshmi/variant_calling/scripts/gencode.v34lift37.annotation.nochr.gtf"
...
build_reference_index_previous() {
    local ref_genome="$1"
    local aligner="$2"
...
build_reference_index() {
    local ref_genome="$1"
    local aligner="$2"
    local genome_dir=$(dirname "$ref_genome")  # Directory of the genome file
    local genome_name=$(basename "$ref_genome")  # Base name of the genome file
    ...
    if [[ "$aligner" == "bwa" ]]; then
        ...
        local index_prefix="${ref_genome}" 
        local index_dir="${genome_dir}/STAR_index"
        echo "$index_dir"  # For consistency in output
        if [[ ! -d "$index_dir" ]]; then
            mkdir -p $index_dir
            # The --runThreadN option can be adjusted according to the number of available CPU threads
            local cmd="STAR --runMode genomeGenerate --genomeDir $index_dir --genomeFastaFiles $ref_genome --runThreadN 10 --sjdbOverhang 100"
            # Include --sjdbGTFfile if you have annotation files
            cmd+=" --sjdbGTFfile ${GTF_ANNOTFILE}"
            execute_and_time "Build Reference Index" "Building reference index using STAR." "$cmd"
        fi
    else
        echo "Error: Unsupported aligner specified."
        exit 1
...
#    local rg_line="@RG\tID:${sample}_${unique_id}\tSM:$sample\tLB:$instrument\tPL:ILLUMINA"
#    echo "$rg_line"
#}
...
parse_fastq_headers() {
    ...
    local flow_cell_id=$(echo "$first_line" | cut -d ':' -f3)
    local lane=$(echo "$first_line" | cut -d ':' -f4)
    #local sample_id=$(basename "$file" | sed 's/_R[12].*//') 
    local sample_id=${sample} 
...
align_reads() {
    ...
    if [[ "$aligner" == "bwa" ]]; then
        ...
        cmd="bwa mem -M -t 10 -R \"$rgid\" $ref_index $fastq1 ${fastq2:+$fastq2} | samtools view -bS - > $bam_output"
        ...
        # Prepare read file command and read files input
        local read_files_command="zcat"
        local read_files_input="$fastq1"

        if [[ -n "$fastq2" ]]; then
            read_files_input+=" $fastq2"
        fi

        # Extract unique identifier from rgid
        local id_part="${rgid#*:}"
        local sample_id_unique_id="${id_part%%\\t*}"

        # First, remove everything up to and including the first underscore after the colon
        #local temp_part="${rgid#*ID:*_}"

        # Remove everything up to 'ID:'
        local temp_part="${rgid#*ID:}"

        # Now, cut off everything after the first underscore to isolate the section
        local library_id="${temp_part%%_*}"
        ...
        cmd="STAR --runMode alignReads \
             ...
             --outSAMattrRGline ID:$sample_id_unique_id SM:${sample} LB:${library_id} PL:ILLUMINA"
             #--outSAMattrRGline \"$rgid\"'
             #--clip3pAdapterSeq 3 \

        # Move/rename the primary output BAM file to match desired pattern
        #cmd+="; mv ${output_dir}/${sample}/${sample}_Aligned.out.bam $bam_output"
        cmd+="; cp ${output_dir}/${sample}/${sample}_Aligned.out.bam $bam_output"
    fi
...
mark_duplicates() {
    ...
    #local split_bam="${output_dir}/${sample}/${sample}.sort.dup.splitN.bam"
    #echo "${dedup_bam}"
    if [[ "$assaytype" == "RNA-seq" ]]; then
        echo "${dedup_bam}"
        ...
        #if [[ ! -s "$split_bam" ]]; then
        if [[ ! -s "$dedup_bam" ]]; then
            #execute_and_time "Mark Duplicates and Split N Cigar Reads" "Marks duplicate reads and splits N Cigar reads in the BAM file for RNA-seq." "$cmd"
...
# Function for marking duplicate reads with Picard
mark_duplicates_plus_splitNCigaRreads() {
    local sample="$1"
    local sorted_bam="$2"
    local output_dir="$3"
    local dedup_bam="${output_dir}/${sample}/${sample}.sort.dup.bam"
...
post_base_quality_recalibration() {
    ...
    local output_dir="$3"
    local outdir="${output_dir}/bqsr_results/${sample}"
...
variant_quality_score_recalibration() {
    ...
    local outdir="${output_dir}/${sample}/results"
    local hardfilter_vcf="${outdir}/${sample}.snp.hardfilter.vcf"
    ...
    if [[ "${assaytype}" == "RNA-seq" ]]; then 
        echo "${hardfilter_vcf}"
        # Perform hard filtering intead of vqsr
        ...
        if [[ ! -s "${hardfilter_vcf}" ]]; then
            #-an MQ -an MQRankSum -an ReadPosRankSum -an FS -an SOR \
            execute_and_time "Variant Hard Filtering" "Running hard filtering thresholds for SNPs." "$cmd"
       fi 
...
normalize_dedup_vcf() {
    ...
    if [[ ! -s "${output_vcf}" ]]; then
        # Define the normalization cmd - D option deprecated and replaced by '-d none'
        #local cmd="bcftools norm -D -o ${output_vcf} -O v ${input_vcf}"
        local cmd="bcftools norm -d none -o ${output_vcf} -O v ${input_vcf}"
...
run_beagle_for_all_chromosomes() {
    ...
    case "$impute_option" in
        ...
        "both") impute_actions=("false" "true") ;;
...
qcanalyze_bam_file() {
    ...
    outdir=$output_dir/qc_results/${sample}
    mkdir -p "$outdir"
...
main_function(){
    ...
    #create_dir_if_not_exists "${output_dir}/$sample"
    ...
    if [[ "$genome_index" == "true" ]]; then
        ...
        output_dir="${OUTFILE_DIR}/sample_outputs"
        create_dir_if_not_exists "${output_dir}/$sample"
        #create_dir_if_not_exists "${output_dir}"
        ...
        bam_file_full=$(align_reads "$ref_index" "$fastq1" "$fastq2" "$aligner" "$output_dir" "$parsed_rgid")
        bam_file=$(echo "$bam_file_full" | grep -o -m 1 '/[^ ]*')
        echo -e "\n\nBam filename: $bam_file"

        sorted_bam=$(sort_bam "$sample" "$bam_file" "$output_dir")
        echo -e "\n\nSorted bam filename: $sorted_bam"

        # IF INTERESTED IN SPLINCIGARREADS Of BAM FILE, esp. RNA-seq case: please run mark_duplicates_plus_splitNCigaRreads() function
        #dedup_bam_full=$(mark_duplicates_plus_splitNCigaRreads "$sample" "$sorted_bam" "$output_dir")
        dedup_bam_full=$(mark_duplicates "$sample" "$sorted_bam" "$output_dir")
        dedup_bam=$(echo "$dedup_bam_full" | grep -o -m 1 '/[^ ]*')
        ...
        recal_bam_full=$(base_quality_recalibration "$sample" "$dedup_bam" "$output_dir")
        #recal_bam=$(echo "$recal_bam_full" | grep -E '/')
        recal_bam=$(echo "$recal_bam_full" | grep -o '/[^ ]*')
        echo -e "\n\nRecalibrated bam_file: $recal_bam"

        gvcfs=$(variant_calling "$sample" "$recal_bam" "$output_dir")
        echo -e "\n\nGenotype calling from GVCFs: $gvcfs"

        genotype_vcf=$(genotype_gvcf "$sample" "$gvcfs" "$output_dir" "single")
        ...
        vqsr_recal_vcf_full=$(variant_quality_score_recalibration "$sample" "$genotype_vcf" "$output_dir")
        #vqsr_recal_vcf=$(echo "$vqsr_recal_vcf_full" | grep -E '/')
        vqsr_recal_vcf=$(echo "$vqsr_recal_vcf_full" | grep -o '/[^ ]*')
        echo -e "\n\nVQSR output file: $vqsr_recal_vcf"
        ...
        echo -e "\nProcessing fastqc report for sample: $sample"
        #generate_qc_reports "$sample" "$fastq1" "$fastq2" "$output_dir"
        generate_qc_reports "$sample" "$fastq1" "$fastq2" "${OUTFILE_DIR}"
        echo -e "\nFastqc report for sample: $sample completed ..."

        # Perform post-base quality recalibration
        ##post_base_quality_recalibration "$sample" "$recal_bam" "${OUTFILE_DIR}"
        ##echo -e "\n\nPost base quality score recalibration and visualization: $recal_bam"
        
        #qcanalyze_bam_file=$(qcanalyze_bam_file "${sample}" "$dedup_bam" "$output_dir")
        qcanalyze_bam_file=$(qcanalyze_bam_file "${sample}" "$dedup_bam" "${OUTFILE_DIR}")
        echo -e "\n\nQC deduped bam_file: $dedup_bam"
